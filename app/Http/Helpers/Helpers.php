...
function lastTen($user_id){
    return $lastTen = BetInvest::where('user_id',$user_id)->orderBy('id','DESC')->get();
}
function historicalWin($user_id){
    $won = BetInvest::where('user_id',$user_id)->where('status',1)->count();
    $made = BetInvest::where('user_id',$user_id)->count();
    try{
        $result = $won / $made;
        return $result.'%';
    }
...
function totalWinnings($user_id){
    $sum = BetInvest::where('user_id',$user_id)->where('status',1)->sum('invest_amount');
    return $sum;
}
